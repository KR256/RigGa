...
class Main(om.MPxCommand):
    ...
    def doIt(self, args):
        ...
        self.NOISE_OR_DENOISE = argVals[3]

        print "Args... CTLid: %s neutralFrame %i ... other id str %s... noiseFlag: %s \n" \
              % (ctlId,self.NEUTRAL_TIME, self.OTHER_FACE_IDS, self.NOISE_OR_DENOISE)
        ...
        guiTemp = resampleCurvesGUI.GUI(CTL_TREE,allStartingWeights,allNeutralWeights,allCurrentGenWeights,
                      strongestShapes,minMaxWeights,allSymmetryNames,self.OTHER_FACE_IDS, self.NOISE_OR_DENOISE)
    ...
    def parseArguments(self, args):
        ...
        noiseOrDenoiseLong = '-noiseOrDenoise'
        noiseOrDenoiseShort = '-nod'
        ...
        if argData.isFlagSet(otherFaceIdsLong):
            ...
            noFlag = False
        if argData.isFlagSet(noiseOrDenoiseLong):
            # In this case, we print the passed flags's three parameters, indexed from 0 to 2.
            flagParams.append(argData.flagArgumentString(noiseOrDenoiseLong, 0))
            noFlag = False
        if noFlag:
            sys.stderr.write(
